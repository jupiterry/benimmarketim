  ...
  return (
    ...
    <motion.div
      ...
      {/* Minimum Sipariş Tutarı İlerleme Çubuğu */}
        ...
        <motion.div 
          className="bg-gradient-to-br from-blue-500/10 via-indigo-500/10 to-purple-500/10 rounded-2xl p-6 border border-blue-500/20"
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-semibold text-blue-400">Ücretsiz Kargo</h3>
          </div>
          
          {(() => {
            const FREE_SHIPPING_THRESHOLD = 300;
            const remainingForFreeShipping = FREE_SHIPPING_THRESHOLD - total;
            const freeShippingProgress = (total / FREE_SHIPPING_THRESHOLD) * 100;
            ...
            return (
              <div className="space-y-3">
                ...
                {remainingForFreeShipping > 0 ? (
                  <div className="text-center space-y-2">
                    <motion.div 
                      className="inline-flex items-center gap-2 bg-blue-500/10 px-4 py-2 rounded-xl border border-blue-500/20"
                      animate={{ scale: [1, 1.02, 1] }}
                      transition={{ duration: 2, repeat: Infinity }}
                    >
                      <span className="text-blue-300">🚚</span>
                      <span className="text-blue-300">Ücretsiz kargo için</span>
                      <span className="text-xl font-bold text-blue-400">₺{remainingForFreeShipping.toFixed(2)}</span>
                      <span className="text-blue-300">daha ekleyin!</span>
                    </motion.div>
                    <p className="text-xs text-gray-400">₺{FREE_SHIPPING_THRESHOLD} ve üzeri siparişlerde kargo ücretsiz</p>
                  </div>
                ) : (
                  <div className="text-center">
                    <motion.div 
                      className="inline-flex items-center gap-2 bg-gradient-to-r from-green-500/20 to-emerald-500/20 px-4 py-2 rounded-xl border border-green-500/30"
                      initial={{ scale: 0.9 }}
                      animate={{ scale: 1 }}
                      transition={{ type: "spring", stiffness: 200 }}
                    >
                      <span className="text-2xl">🎉</span>
                      <span className="text-green-400 font-semibold">Tebrikler! Ücretsiz kargo kazandınız!</span>
                      <span className="text-2xl">🚚</span>
                    </motion.div>
                  </div>
                )}
