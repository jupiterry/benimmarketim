    ...
    if (user?.role === "admin") {
      ...
      socket.on("newOrder", (data) => {
        ...
        toast.custom(
          (t) => (
            <div
              className={`${t.visible ? "animate-enter" : "animate-leave"} max-w-md w-full bg-gray-800 shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5`}
            >
                      ...
                      <p className="flex items-center gap-1">
                        <span
                          className={`w-2 h-2 rounded-full ${
                            data.order.status === "Hazırlanıyor"
                              ? "bg-yellow-500"
                              : data.order.status === "Yolda"
                              ? "bg-blue-500"
                              : data.order.status === "Teslim Edildi"
                              ? "bg-emerald-500"
                              : "bg-red-500"
                          }`}
                        />
            ...
            <div className="max-h-96 overflow-auto">
              {notifications.map((notif, index) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-400 mt-1">Tutar: ₺{notif.order.totalAmount}</p>
